...
module.exports.post = async function(req,res){
    ...
    try{
        var user = await AccountSchema.findOne({'phonenumber': req.body.user.phonenumber})
        if(user != null)
        {
            console.log("balance")
            ...
            if(user.balance >= req.body.cardAmount * req.body.cardCount)
            {
                ...
                var waitingBets = await BetSchema.find({phonenumber:user.phonenumber,betOrdered:false})
                /**bonus to promotion */
                ...
                if(parent)
                {
                    await PromotionBonus.create({
                        ...
                        betType     :   req.body.type,
                        balance     :   req.body.cardAmount * req.body.cardCount,
                        promotionLevel  :   1
                    })
