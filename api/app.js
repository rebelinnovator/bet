...
async function reBalance()
{
  ...
  //  console.log(bets)
    ...
    for(i = 0;i < bets.length;i++)
    {
        ...
        {
            console.log("9(((((BETCARD")

            if(bettingTotalResult[bets[i].betType].Num == bets[i].betCardType)   
            {
                //success 
                //in this case user wiil get += bidamount * 9
                
           //     console.log("BETCARD")
          //      console.log(bets[i])
                earn = 9 * (bets[i].betCardAmount * bets[i].betCardCount * 0.95)
                user.balance = user.balance + 9 * (bets[i].betCardAmount * bets[i].betCardCount * 0.95)
                await user.save()
            }
        }else
        {
...
async function emitNewBetting(socket,userid,bettingHistory)
{
    ...
    if(user){
        ...
        if(bettingHistory == -100){
            ...
            waitingBets = await BetSchema.find({phonenumber:user.phonenumber,betOrdered:false})
        }
